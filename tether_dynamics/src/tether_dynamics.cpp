    ...
    a_(4.0), nh_(), nh_private_("~"), tfBuffer_(), tfListener_(tfBuffer_)
{
    ...
    tether_length_sub_ = nh_.subscribe("sim_tether_limit", 1, &TetherDynamics::tetherLengthCallback, this);
...
//    L_lim_ = nh_private_.param<double>("tether_limit", 20.0);
    L_lim_ = nh_private_.param<double>("tether_takeoff_length", 0.2);
    reel_speed_ = nh_private_.param<double>("reel_speed", 0.5);
...
void TetherDynamics::tetherLengthCallback(const std_msgs::Float64 &msg)
{
    static double dL = 0.05; // m
    double desired_length = msg.data;
    double Hz = reel_speed_ / dL;
    ROS_WARN("[Tether Dynamics Node] Changing tether length to %f meters.", desired_length);
    if (desired_length > L_lim_)
        L_lim_ = desired_length;
    else
    {
        while (abs(L_lim_ - desired_length) > dL)
        {
            L_lim_ += dL * sgn(desired_length - L_lim_);
            ros::Rate(Hz).sleep();
        }
    }
    ROS_WARN("[Tether Dynamics Node] Done.");
}
...
void TetherDynamics::onUpdate(const ros::TimerEvent &event)
{
    ...
    if (X_ < 0.5 || Y_ < 0.5)
    {
        ROS_WARN_THROTTLE(10.0, "[Tether Dynamics Node] Tether bounds too small to proceed.");
        return;
    }
    ...
    {
        ROS_WARN_THROTTLE(1.0, "[Tether Dynamics Node] Tether curvature could not be estimated.");
        return;
    }
...
void TetherDynamics::calculateBaselines()
{
    ...
    Force_(1) *= -1.0; // get into actual boat frame
    Force_(2) *= -1.0;
...
void TetherDynamics::computeControl()
{
    ...
    Vector3d unit = q_BOAT_UAV_.R() * p_boat2UAV_boat.normalized(); // uav body frame
