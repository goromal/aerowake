...
def draw_axes(img, imgpts):
    ...
    cv2.line(img_deep_copy,tuple(imgpts[0].ravel()),tuple(imgpts[1].ravel()),85,3)  # x-axis (darkest)
...
class CentroidFinder(object):
  ...
  def _obtain_initial_centroids(self):
    ...
    if self._debug > 1:
      print text_colors.BOLD + "# Contours = " + str(len(contours)) + text_colors.ENDCOLOR
    if len(contours) <= 20: #12:
...
class VisionPose(object):
  def __init__(self, mtx, dist, objp, debug, bin_thresh, rotate, logdir):
    ...
    self.logdir = logdir
    self.debug = debug
    axlen = np.linalg.norm(np.array(objp[0:3])-np.array(objp[3:6]))
